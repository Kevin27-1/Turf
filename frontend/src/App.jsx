...
import { getFirebaseAuth, signInWithGoogle } from './firebase.js';
...
export default function App() {
  ...
  const [authMode, setAuthMode] = useState('login'); // 'login' | 'signup' | 'forgot' | 'google_phone'
  const [signupStep, setSignupStep] = useState('details'); // 'details' | 'otp'
  const [forgotStep, setForgotStep] = useState('phone'); // 'phone' | 'otp' | 'reset'
  const [googleStep, setGoogleStep] = useState('phone'); // 'phone' | 'otp'
  const [googleAuthData, setGoogleAuthData] = useState(null); // { idToken, googleUser }
  ...
  const handleOpenBooking = async (slot) => {
    ...
    try {
      ...
      // Mouse pointer check (desktop browsers have 'fine' pointer, mobile/tablets have 'coarse' or 'none')
      const isCoarsePointer = window.matchMedia && window.matchMedia('(pointer: coarse)').matches;

      const deviceType = isMobileUA || isCoarsePointer ? 'mobile' : 'desktop';
  ...
  const handleSendSignUpOtp = async (e) => {
    ...
    try {
      const checkRes = await fetch('/api/auth/signup/check-phone', {
        method: 'POST',
      ...
      if (!checkRes.ok) {
        throw new Error(checkData.error || 'Phone number is already registered');
      }
  ...
  const handleVerifySignUpOtp = async (e) => {
    ...
    if (!confirmationResult) {
      setAuthFormError('Verification session expired. Please go back and try again.');
    ...
    try {
      ...
      if (err.code === 'auth/invalid-verification-code') {
        ...
        errMsg = 'Verification code has expired. Please resend OTP.';
      }
      setAuthFormError(errMsg);
    } finally {
      setAuthFormLoading(false);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { idToken, user: gUser } = await signInWithGoogle();
      ...
      // If first-time Google user or needs mobile number verification
      if (data.needsPhone) {
        setGoogleAuthData({
          idToken,
        ...
        setAuthName(data.googleUser?.name || gUser.displayName || '');
        setAuthPhone('');
        setGoogleStep('phone');
        setAuthMode('google_phone');
        setAuthFormSuccess('Google account verified! Please enter your mobile number to complete registration.');
        return;
      }

      // Existing user with verified phone: log in immediately
      localStorage.setItem('jwt_token', data.token);
      localStorage.setItem('user_profile', JSON.stringify(data.user));
      setUser(data.user);
      setProfile({
        ...
        phone: data.user.phone ? `+91 ${data.user.phone}` : ''
      });
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        errMsg = 'Google popup was closed before completing sign-in. Please try again.';
      } else if (err.code === 'auth/cancelled-popup-request') {
        errMsg = 'Only one sign-in popup can be active at a time.';
      } else if (err.code === 'auth/popup-blocked') {
        errMsg = 'Sign-in popup was blocked by your browser. Please allow popups for this site.';
      } else if (err.code === 'auth/unauthorized-domain') {
        errMsg = 'Current domain is not authorized in Firebase Console. Add it in Auth > Settings > Authorized domains.';
      }
      setAuthFormError(errMsg);
    } finally {
      setAuthFormLoading(false);
  ...
  const handleSendGooglePhoneOtp = async (e) => {
    ...
    const cleanPhone = authPhone.replace(/\D/g, '');
    if (!cleanPhone || cleanPhone.length < 10) {
      setAuthFormError('Please enter a valid 10-digit mobile number');
      return;
    }

    setAuthFormLoading(true);
    try {
      // 1. Check if phone is available
      const checkRes = await fetch('/api/auth/signup/check-phone', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone: cleanPhone })
  ...
  const handleVerifyGooglePhoneOtp = async (e) => {
    e.preventDefault();
    setAuthFormError('');
    setAuthFormSuccess('');
    ...
    if (!confirmationResult || !googleAuthData?.idToken) {
      setAuthFormError('Verification session expired. Please restart Google sign in.');
      setAuthMode('login');
      return;
    }
    ...
    try {
      ...
      const res = await fetch('/api/auth/google/complete', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          googleToken: googleAuthData.idToken,
          phoneToken,
          name: authName.trim() || googleAuthData.googleUser?.name || 'Player',
          phone: authPhone.replace(/\D/g, '')
        })
      });
      ...
      console.error('Google registration completion failed:', err);
      let errMsg = err.message;
      if (err.code === 'auth/invalid-verification-code') {
        errMsg = 'Invalid verification code. Please check the 6-digit OTP and try again.';
      } else if (err.code === 'auth/code-expired') {
            ...
            <div className="border border-neutral-900 bg-neutral-950/50 p-6 space-y-6 my-4 animate-in fade-in duration-200">
              ...
              <div className="text-center">
                <h3 className="text-base font-black text-white uppercase tracking-tight">
                  ...
                  {authMode === 'google_phone' && (googleStep === 'phone' ? 'Verify Mobile Number' : 'Enter 6-Digit OTP')}
                  {authMode === 'forgot' && (
                ...
                <p className="text-[10px] text-neutral-400 mt-1 uppercase font-bold tracking-wider">
                  ...
                  {authMode === 'google_phone' && (
                    googleStep === 'phone' 
                      ? 'Link your mobile number to complete Google registration' 
                      : `Enter the 6-digit verification code sent to +91 ${authPhone}`
                  )}
                  {authMode === 'forgot' && (
              ...
              {authMode === 'login' && (
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  <div className="relative my-3 flex items-center justify-center">
                    ...
                    <span className="relative bg-neutral-950 px-3 text-[9px] font-bold text-neutral-500 uppercase tracking-wider">
                      Or continue with
                    </span>
                  </div>
                  ...
                  <button 
                    ...
                    disabled={authFormLoading}
                    className="w-full bg-neutral-900 border border-neutral-800 hover:border-neutral-700 hover:bg-neutral-850 text-white font-bold text-xs p-3.5 flex items-center justify-center gap-3 transition cursor-pointer disabled:opacity-50 shadow-[2px_2px_0px_#000]"
                  >
                ...
                <>
                  ...
                  {signupStep === 'otp' && (
                    <form onSubmit={handleVerifySignUpOtp} className="space-y-4">
                      <div className="space-y-1.5">
                        <label className="block text-[9px] font-bold text-neutral-500 uppercase tracking-wider">6-Digit Verification OTP</label>
                        <input 
                          type="text" 
                          required
                          value={authOtpCode}
                      ...
                      <div className="text-center pt-2">
                        ...
                        >
                          Change Registration Details / Phone
                        </button>
                      </div>
                    </form>
                  )}
              ...
              {/* GOOGLE PHONE VERIFICATION FORM */}
              {authMode === 'google_phone' && (
                <div className="space-y-4">
                  {/* Google Profile Card Header */}
                  <div className="border border-neutral-800 bg-neutral-900/60 p-3.5 flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      {googleAuthData?.googleUser?.picture ? (
                        <img 
                          src={googleAuthData.googleUser.picture} 
                          alt="Google Profile" 
                          className="w-9 h-9 rounded-full border border-neutral-700 object-cover shrink-0" 
                        />
                      ) : (
                        <div className="w-9 h-9 rounded-full bg-blue-500/20 text-blue-400 font-black flex items-center justify-center text-xs shrink-0">
                          G
                        </div>
                      )}
                      <div className="min-w-0">
                        <div className="text-xs font-bold text-white flex items-center gap-1.5 truncate">
                          <span className="truncate">{googleAuthData?.googleUser?.name || 'Google User'}</span>
                          <CheckCircle className="w-3.5 h-3.5 text-[#22c55e] shrink-0" />
                        </div>
                        <div className="text-[10px] text-neutral-400 font-mono truncate">
                          {googleAuthData?.googleUser?.email}
                        </div>
                      </div>
                    </div>
                    <span className="text-[9px] font-black uppercase tracking-wider text-[#22c55e] bg-[#22c55e]/10 px-2 py-1 border border-[#22c55e]/30 shrink-0">
                      Google Verified
                    </span>
                  </div>
                  ...
                  {googleStep === 'phone' && (
                    <form onSubmit={handleSendGooglePhoneOtp} className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        <label className="block text-[9px] font-bold text-neutral-500 uppercase tracking-wider">Mobile Number (For Booking OTP & SMS)</label>
                        <div className="relative">
                          <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-neutral-500 text-xs font-bold">+91</span>
                          <input 
                            type="tel" 
                      ...
                      <div className="text-center pt-2 border-t border-neutral-900">
                        <button
                          ...
                          className="text-neutral-400 hover:text-white text-xs font-bold underline cursor-pointer"
                        >
                    ...
                    <form onSubmit={handleVerifyGooglePhoneOtp} className="space-y-4">
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-neutral-900">
                        ...
                        <button
                          ...
                          disabled={authFormLoading}
                          className="text-[#22c55e] hover:underline text-xs font-bold cursor-pointer disabled:opacity-50"
                        >
                          Resend OTP
                        </button>
                      </div>
